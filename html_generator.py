...
====================
Genera index.html avanzado con:
- Carvana Setup destacado (badge ⭐ + filtro dedicado)
- Columnas extras: PER, IPO years, Short %, Revenue growth, Upside %
- Filtros: signal, cap tier, sector, Carvana Setup, PER max, IPO recientes,
  short interest mínimo, drawdown mínimo, búsqueda
- Modal de detalle al hacer click en cualquier fila con TODO el desglose
- Razones del score Carvana visibles
...
<style>
  ...
  /* Hide scrollbar in modal for cleaner look */
...
<div id="content" class="hidden">
  ...
  <div class="max-w-7xl mx-auto px-4 py-4">
    <div class="card rounded-lg p-3 mb-3">
      ...
      <!-- Filtros avanzados -->
      <div class="mt-3 pt-3 border-t border-neutral-800 flex gap-2 flex-wrap items-center text-sm">
        <span class="text-neutral-500">Filtros avanzados:</span>

        <label class="flex items-center gap-1 cursor-pointer">
          <input id="carvanaOnly" type="checkbox" onchange="applyFilters()"
            class="accent-yellow-500">
          <span>⭐ Solo Carvana Setups</span>
        </label>
  ...
  <footer class="text-center py-6 text-xs text-neutral-600">
    Generado el {{scan_date_human}} · CB Scanner v2.0
  </footer>
</div>
...
function render() {
  ...
  tbody.innerHTML = filteredData.map((r, idx) => {
    ...
    return '<tr class="border-t border-neutral-800 row-clickable ' + (isCarvana ? 'row-carvana' : '') + '" onclick="openDetail(' + idx + ')">'
      ...
      + '<td class="px-2 py-2 text-neutral-400 max-w-[180px] truncate">' + (r.name || '') + '</td>'
      + '<td class="px-2 py-2"><span class="badge ' + tb.cls + '">' + tb.label + '</span></td>'
      + '<td class="px-2 py-2 text-neutral-400 text-xs max-w-[140px] truncate">' + (r.sector || '') + '</td>'
    ...
    <div class="card p-3 rounded mb-4">
      ...
      <div class="grid grid-cols-3 gap-3 text-center">
        ...
        <div>
          ...
          <div class="text-sm font-bold uppercase">${r.recommendation || '—'}</div>
        </div>
      </div>
    </div>
...
if __name__ == "__main__":
    print("═" * 60)
    print("  CB SCANNER — Generador HTML v2")
    print("═" * 60 + "\n")
    generate_html()
